...
intersect_point::intersect_point()
{
	...
	robot_pose_sub = n.subscribe<geometry_msgs::Pose2D>("/robot_position", 5, &intersect_point::robotCallBack, this);
...
void intersect_point::robotCallBack(const geometry_msgs::Pose2D::ConstPtr& robot_pose)
{
	...
	static float x_raw[20], y_raw[20];
	static int cnt=0;
	if(cnt<20){
		x_raw[cnt] = robot_pose->x;
		y_raw[cnt] = robot_pose->y;
	}
	cnt++;
	double time_now = ros::Time::now().toSec();
	if(time_now - last_time > 0.2){
		float avrg_x, avrg_y;
		for(int i = 0; i < cnt; i++){
			avrg_x += x_raw[i];
			avrg_y += y_raw[i];
		...
		robot.x_history[0] = avrg_x;
		for(int i=9;i>0;i--){
			robot.y_history[i] = robot.y_history[i-1];
		}
		robot.y_history[0] = avrg_y;
	}
//	ROS_INFO("Received");
...
void intersect_point::calc_intersect(robot_pose robot, const robot_pose drone)
{
	...
	//else
	{
		...
		//	float b = robot.y - robot.x * k;
			...
			float k = KB(0);
			float b = KB(1);
			ROS_INFO("\nx1: %f, x2: %f, x3: %f, x4: %f",robot.x_history[0],robot.x_history[1],robot.x_history[2],robot.x_history[3]);
			ROS_INFO("\ny1: %f, y2: %f, y3: %f, y4: %f",robot.y_history[0],robot.y_history[1],robot.y_history[2],robot.y_history[3]);
			ROS_INFO("\nk: %f\nb: %f", k, b);
        ...
        //	if (left_intersect_x < -HEIGHT || right_intersect_x < -HEIGHT)
            if(1)
            {
				if(robot.x_history[3]>robot.x_history[0]){
                    ...
                    intersect.y = k * intersect.x + b;//if y close in 2 times let it stay still
                    if(fabs(intersect.y) > WIDTH / 2){
                        intersect.y = 0;
                    }
                    if(fabs(intersect.y - last_y) < 0.1){
                        intersect.y = last_y;
                    }
                    else{
                        last_y = intersect.y;
